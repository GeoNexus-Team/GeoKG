...
### 收尾：术语表 371 条 + SDG 指标核对（已完成）

**术语表（原 CONCEPTS + EXTENDED_CONCEPTS，371 条）** —— 审计把它们一概称为
"来源未核实"其实不准确。它们的性质分三种，混在一起才是问题：

| 处理 | 类别 | 条目 | 结果 |
|------|------|-----:|------|
| **改由权威源提供** | Sensors | 56 | → WMO OSCAR 仪器目录（**1,244** 台，T2） |
| **删除（已被覆盖）** | DisasterRisk | 21 | → IRDR 灾害分类（L3）已覆盖，避免两套口径 |
| **引外部来源** | CoordinateSystems / Standards / PolicyFrameworks / SDGFramework | 76 | → `iogp-epsg` / `iso-ogc-standards` / `un-frameworks` |
| **标注为自编术语表** | 其余 9 类 | 218 | → `geokg-authored`（Apache-2.0） |

**最后一类是关键**：把"缓冲区分析""坐标参考系"这类通用术语标为
``geokg-authored``，表示"**这是我们编的编辑内容**"，而不是"我们不知道它从哪来"。
这是**如实分类，不是免除引用义务**——凡取自外部数据集的内容都不在此列
（仪器与灾害术语已分别改由 OSCAR 与 IRDR 提供）。

**SDG 指标核对** —— 发现两处问题，均已修正：

| 问题 | 说明 | 处理 |
|------|------|------|
| 文档数字过时 | 声称"231 个唯一指标"，那是 2017 年 A/RES/71/313 的数 | 官方 API 现为 **251** |
| 手写表少 7 个 | Goal 2 / 7 / 10 / 11 / 16 各有缺口（244 vs 251） | 改用官方 API 生成 |
| **id 是伪造的** | 旧实现按"每目标有 N 个"用 ``range(1, N+1)`` **编造 id**，于是 ``1.a``/``2.a``/``5.c``/``17.18`` 这类真实代码全被错编号 | 改用**官方代码**，并加测试护栏 |

顺带补上了指标的**说明与层级（tier）**——170 条 tier-1 / 81 条 tier-2，
这才是 SDG 监测真正要用的字段。
...
|------|---------:|-----:|
| 实体总数（口径②） | 8,327 | **7,382** |
| **来源已核实** | 703（9.3%） | **7,382（100%）** |
| 来源未核实 | 6,887 | **0** |

已在 `scripts/verify_tier1_kpis.py` 与 `scripts/counting_basis.py` 中把口径
随结论一并输出，达标与否不再是孤立的 ✅。
